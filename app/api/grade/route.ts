...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `
You are an expert IELTS examiner. Grade the following IELTS Writing Task 2 essay.
...
FIRST, evaluate if the essay is relevant to the given topic and instructions.
If the essay is completely unrelated to the topic or does not address the task instructions:
1. Return a JSON response with all scores set to 1
2. Include feedback explaining that the essay is off-topic
3. Set a special flag "is_off_topic" to true

If the essay is at least partially related to the topic, proceed with grading according to the official IELTS Writing Task 2 Band Descriptors:
...
- Band 7: Addresses all parts of the task; presents a clear position throughout the response; presents, extends and supports main ideas, but there may be a tendency to over-generalize and/or supporting ideas may lack focus
...
- Band 5: Addresses the task only partially; expresses a position but the development is not always clear and there may be no conclusions drawn; presents some main ideas but these are limited and not sufficiently developed; there may be irrelevant detail
- Band 4: Responds to the task only in a minimal way or the answer is tangential; the position may be unclear; presents few ideas, which are largely undeveloped or irrelevant
...
`
    ...
    try {
      ...
      try {
        ...
        // Calculate overall score (average of all criteria scores, rounded to nearest 0.5)
        const scores = [
          result.criteria.taskResponse.score,
          result.criteria.coherenceAndCohesion.score,
          result.criteria.lexicalResource.score,
          result.criteria.grammaticalRangeAndAccuracy.score
        ]
        const average = scores.reduce((a, b) => a + b, 0) / scores.length
        result.overallScore = Math.round(average * 2) / 2 // Round to nearest 0.5
        
        // Store the result
        const storeResponse = await fetch(new URL('/api/results', req.url).toString(), {
          method: 'POST',
          headers: {
